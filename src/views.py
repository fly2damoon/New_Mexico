...
from .forms import LoginForm, TeamAddForm, KidAddForm
...
@app.route('/teams/<int:page>', methods=['GET', 'POST'])
@login_required
def teams(page=1):
    """Page - All New Mexico teams"""
    teamaddform = TeamAddForm()
    team =  teamaddform.validate_on_submit()
    if team:
    ...
    return render_template('teams.html', title="Mission Teams", \
                            teams=teams, teamaddform=teamaddform)
